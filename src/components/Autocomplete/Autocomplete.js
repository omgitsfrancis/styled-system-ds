...
export const Autocomplete = ({ children, onSelect, defaultValue = "" }) => {
  ...
  return (
    <AutocompleteContext.Provider value={contextValue}>
      <AutocompleteDiv className="autocomplete">{children}</AutocompleteDiv>
...
export const AutocompletePopover = ({ children, ...props }) => {
  ...
  const Popover = styled.div`
    border: solid 0px;
    border-top: 0;
    box-shadow: 0 0.25rem 0.25rem 0.2rem black;
...
export const AutocompleteList = ({ children }) => {
  ...
  return (
    <StyledList className="autocomplete__list" role="list">
      {children}
    </StyledList>
  );
};
